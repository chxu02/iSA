...
bamToBed -bedpe -i $1.Wat.bam | awk -v b1=$2 -v b2=$3 '{if($2<b1){$9=0} else{$9=$2-b1}; print $1,$9,$6+b2,NR,$7}' OFS='\t' | sort -k1,1 -k2,2n -k3,3n -u -S 64G > $1.Wat.bed;
echo Writing bed file for Crick strand...
bamToBed -bedpe -i $1.Cri.bam | awk -v b1=$2 -v b2=$3 '{if($2<b2){$9=0} else{$9=$2-b2}; print $1,$9,$6+b1,NR,$7}' OFS='\t' | sort -k1,1 -k2,2n -k3,3n -u -S 64G > $1.Cri.bed;
echo Heating...
...
Y|y) echo Continue on... Extracting paired alignments...
     ...
     echo Ignoring the first $4 bases of mate 1 reads, | tee -a $1.iSA.report.txt
     echo Ignoring the first $5 bases of mate 2 reads. | tee -a $1.iSA.report.txt
     bismark_methylation_extractor -p --multicore 8 --merge_non_CpG --no_header --gzip $1.Wat.iSA.bam
     bismark_methylation_extractor -p --multicore 8 --merge_non_CpG --no_header --gzip $1.Cri.iSA.bam
...
Y|y) echo Cleaning up...
     rm $1.Wat.bam $1.Cri.bam $1.Wat.bed $1.Cri.bed $1.Wat.LN $1.Cri.LN header $1.Wat.iSA.M-bias_R?.png $1.Cri.iSA.M-bias_R?.png $1.Wat.ID $1.Cri.ID $1.Wat.me $1.Cri.me;;
N|n) exit;;
esac
